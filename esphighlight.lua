...
local function loadPlayer(v)
	...
	-- تحميل فوري إذا Character موجود
	if v.Character then
		setupESP(v)
...
-- ⭐ النظام الجديد: فحص وإصلاح تلقائي كل ثانية
	...
	for _, v in pairs(players:GetPlayers()) do
		if v ~= plr and v.Character then
			...
			if humanoid and humanoid.Health > 0 then
				-- فحص وإصلاح الـ NameTag
				pcall(setupESP, v)
				-- فحص وإصلاح الـ Highlight
				pcall(setupHighlight, v)
			end
		end
	end
end)